...
| 工程化量化评测 | 7 条官方原始问答 + 60 条基础回归题 + 30 条独立扩展题，共 97 条；记录可执行率、结果准确率、一次通过率、修复后通过率、耗时与失败原因 | 评测中心可分开运行扩展集、生成复核队列、导出复核包并回写裁定 |
...
- 全部合计：97 条激活案例；其中核心基线为 67 条（简单 23、中等 31、复杂 6），扩展集为简单 14、中等 12、复杂 4。
...
- **评测中心**：运行全部 97 题，或单独运行 30 题独立扩展集，观察质量指标并创建/导出/回写人工复核批次。
...
GitHub Actions 会在 `main` 推送和 Pull Request 时自动执行上述后端静态检查/测试与前端 lint/build，工作流见 [.github/workflows/ci.yml](.github/workflows/ci.yml)。模型调用型 97 题全量回归会消耗 API 额度；可先单独运行 30 题扩展集，再按版本运行全量评测，并将真实结果作为发布依据。
...
backend/
  ...
  db/                  官方数据导入器、97 题回归生成器、schema 与迁移文件
