...
| 工程化量化评测 | 7 条官方原始问答 + 60 条由同一官方表实跑生成的回归题，共 67 条；记录可执行率、结果准确率、一次通过率、修复后通过率、耗时与失败原因 | 评测中心可发起运行、生成复核队列、导出复核包并回写裁定 |
...
- 基线合计：67 条激活案例；复杂度分布为简单 23、中等 31、复杂 6。
...
- **评测中心**：运行 67 题基线，观察质量指标，创建/导出/回写人工复核批次。
...
GitHub Actions 会在 `main` 推送和 Pull Request 时自动执行上述后端静态检查/测试与前端 lint/build，工作流见 [.github/workflows/ci.yml](.github/workflows/ci.yml)。模型调用型 67 题全量回归会消耗 API 额度，建议在评测中心按版本手动或定时运行，并将真实结果作为发布依据。
...
backend/
  ...
  db/                  官方数据导入器、67 题回归生成器、schema 与迁移文件
