from db.load_official_challenge_v3_cases import CASES, SOURCE_TYPE, validate


def test_third_held_out_challenge_has_thirty_safe_official_cases() -> None:
    validate()

    assert SOURCE_TYPE == "official_challenge_v3"
    assert len(CASES) == 30
    assert len({case.code for case in CASES}) == len(CASES)
    assert len({case.question for case in CASES}) == len(CASES)
    assert all(case.code.startswith("V3-") for case in CASES)
