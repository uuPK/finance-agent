...
METRICS = [
    ...
    (
        "normal_total_asset",
        "普通账户总资产",
        "指定资产快照内普通账户总资产之和，仅统计 nm_tot_aset。",
    ...
    (
        "credit_total_asset",
        "信用账户净资产",
        "指定资产快照内信用账户净资产之和，仅统计 fc_pur_aset。",
    ...
    (
        ...
        "双向交易客户",
        "指定期间内同时存在买入和卖出记录的客户，必须先按客户汇总后过滤。",
    ...
    (
        ...
        "不同产品持有数量",
        "指定持仓快照内每位客户去重后的产品数量，必须先按客户汇总后过滤。",
...
TERMS = [
    ...
    (
        "成交数量",
        "成交数量使用 buy_mnt + sell_mnt，不能按交易金额计算。",
        ["交易数量", "成交份额"],
        False,
    ),
    (
        "交易费用",
        "交易费用使用 buy_fare + sell_fare。",
        ["买卖费用", "手续费"],
        False,
    ),
    (
    ...
    (
        "转账金额",
        "转账金额使用 tran_in + tran_out。",
        ["转入转出金额", "资金转账金额"],
        False,
    ),
    (
        "划拨金额",
        "划拨金额使用 assign_in + assign_out。",
        ["划入划出金额", "资金划拨金额"],
        False,
    ),
    (
        "一级营业部",
        "一级营业部使用 dim_branch.up_org_name；仅要求一级营业部时不额外按 org_name 分组。",
        ["分公司", "上级营业部"],
        False,
    ),
    (
        "客户与事实快照日期",
        "客户属性快照为 20260531；截至 2026-03-31 的资产和持仓只筛选事实表，关联客户属性时不把客户表限定为 20260331。",
        ["客户快照", "资产快照", "持仓快照"],
        False,
    ),
    (
        "双向交易客户",
        "同时发生过买入和卖出是客户期间级条件：先按 pty_id 汇总，分别判断买入金额和卖出金额之和大于零，再在外层统计客户数。",
        ["同时买入和卖出", "买卖双向", "双向交易"],
        False,
    ),
    (
        "不同产品持有数量",
        "持有至少 N 只不同产品是客户快照级条件：先按 pty_id 统计 count(distinct prdt_id) 并在 HAVING 应用阈值，再在外层统计客户数。",
        ["多产品持仓", "至少持有不同产品", "去重产品数量"],
        False,
    ),
    (
        "钻石卡客户",
