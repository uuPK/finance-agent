...
_SQL_CRITIC_SYSTEM_PROMPT = dedent(
    ...
    正例：
    QueryPlan 要求 total_asset > 500000、trade_amount > 500000、2026年一季度、
    customer 粒度。SQL 包含资产 WHERE、交易日期 WHERE、GROUP BY pty_id、
    HAVING SUM(buy_amt + sell_amt) > 500000、LIMIT。
    应输出 passed=true，evidence 包含 filters_covered、time_window_covered、grain_covered。

    反例：
    QueryPlan 要求 trade_amount > 500000，但 SQL 统计了所有日期交易，没有 data_dt 条件。
    应输出：
    {
      "passed": false,
      "score": 55,
      ...
      "reason": "SQL 未实现 QueryPlan 要求的一季度交易窗口。",
      "evidence": ["QueryPlan metric: trade_amount", "SQL has no data_dt time filter"],
      "repair_hint": "为交易表增加 2026 年一季度 data_dt 条件。",
