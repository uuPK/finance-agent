...
_PLAN_CRITIC_SYSTEM_PROMPT = dedent(
    ...
    - metadata context 若提供普通账户总资产、信用账户净资产、成交数量、交易费用、现金净流入、
      转账金额或划拨金额，应认可计划使用相应专属 metric_code，不能要求退化为总资产、交易金额或
      净资金流入。
    - 用户仅要求“按一级营业部”时，up_org_name 是完整分组维度；额外加入 org_name 会改变粒度，
      应判为 wrong_grain。资产/持仓的截至日期不应被错误复制为客户属性表的快照日期。
