...
1. 应用 `backend/db/schema.sql`，创建 `mart`、`metadata`、`agent`、`evaluation` schema。
2. 运行 `backend/db/seed_synthetic_data.py`，生成 500 客户 synthetic 测试数据。
3. 编写只读 SQL Executor，限制超时、行数和语句类型。
4. 接入 SQL 执行日志和阶段日志。
5. 实现 ResultHardValidator。
6. 接入 ResultCritic。
7. 将结果校验失败反馈回 SQL Actor 修复循环。
