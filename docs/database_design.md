...
### 3.1 客户信息表：`mart.dim_customer`

客户维表，用于客户画像、分群、明细查询。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `customer_id` | varchar(32) primary key | 脱敏客户编号 |
| `customer_name` | varchar(64) | 脱敏客户名称，用于演示，不存真实姓名 |
| `gender_code` | varchar(16) | 性别代码 |
| `age` | integer | 年龄 |
| `age_band` | varchar(32) | 年龄段 |
| `region_code` | varchar(32) | 地区代码 |
| `customer_level` | varchar(32) | 客户等级 |
| `risk_level` | varchar(32) | 风险等级 |
| `open_date` | date | 开户日期 |
| `status` | varchar(32) | 客户状态 |
| `is_active` | boolean | 是否有效客户 |
...
### 3.2 服务经理信息表：`mart.dim_manager`

服务经理维表，用于按经理、团队、机构汇总客户和资产。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `manager_id` | varchar(32) primary key | 服务经理编号 |
| `manager_name` | varchar(64) | 脱敏经理名称 |
| `org_id` | varchar(32) | 所属机构编号 |
| `org_name` | varchar(128) | 所属机构名称 |
| `team_name` | varchar(128) | 团队名称 |
| `manager_level` | varchar(32) | 经理等级 |
| `status` | varchar(32) | 在职、离职、停用等 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |
...
### 3.3 产品信息表：`mart.dim_product`

产品维表，用于持仓、交易、产品偏好分析。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `product_id` | varchar(32) primary key | 产品编号 |
| `product_name` | varchar(128) | 产品名称 |
| `product_type` | varchar(32) | 基金、股票、债券、理财等 |
| `risk_level` | varchar(32) | 产品风险等级 |
| `issuer` | varchar(128) | 发行方 |
| `status` | varchar(32) | 在售、停售、到期等 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |
...
### 3.4 服务关系表：`mart.rel_customer_manager`

客户与服务经理的关系表，支持当前归属和历史归属。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `relation_id` | bigserial primary key | 关系编号 |
| `customer_id` | varchar(32) | 客户编号 |
| `manager_id` | varchar(32) | 服务经理编号 |
| `start_date` | date | 关系开始日期 |
| `end_date` | date | 关系结束日期，当前关系为空 |
| `is_current` | boolean | 是否当前服务关系 |
| `relation_type` | varchar(32) | 主服务、协同服务等 |
| `created_at` | timestamptz | 创建时间 |
| `updated_at` | timestamptz | 更新时间 |
...
### 3.5 公共维表：`mart.dim_common_code`

统一维护枚举代码，避免在 prompt 和 SQL 中硬编码中文枚举。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `code_type` | varchar(64) | 代码类型，如 gender、risk_level、customer_level |
| `code` | varchar(64) | 代码值 |
...
建议 `(code_type, code)` 建唯一约束。

### 3.6 客户资产日表：`mart.fact_customer_asset_daily`

客户资产事实表，客户-日期粒度，是营销筛选中最核心的指标来源。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `asset_id` | bigserial primary key | 记录编号 |
| `stat_date` | date | 统计日期 |
| `customer_id` | varchar(32) | 客户编号 |
| `total_asset` | numeric(20, 2) | 总资产 |
| `cash_asset` | numeric(20, 2) | 现金资产 |
| `fund_asset` | numeric(20, 2) | 基金资产 |
| `stock_asset` | numeric(20, 2) | 股票资产 |
| `bond_asset` | numeric(20, 2) | 债券资产 |
| `wealth_asset` | numeric(20, 2) | 理财资产 |
| `available_cash` | numeric(20, 2) | 可用资金 |
| `created_at` | timestamptz | 创建时间 |

建议唯一约束：`(stat_date, customer_id)`。
...
### 3.7 客户持仓日表：`mart.fact_customer_position_daily`

客户-产品-日期粒度，用于产品持仓与潜客识别。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `position_id` | bigserial primary key | 记录编号 |
| `stat_date` | date | 统计日期 |
| `customer_id` | varchar(32) | 客户编号 |
| `product_id` | varchar(32) | 产品编号 |
| `holding_amount` | numeric(20, 2) | 持仓金额 |
| `holding_share` | numeric(20, 4) | 持仓份额 |
| `market_value` | numeric(20, 2) | 市值 |
| `profit_amount` | numeric(20, 2) | 持仓盈亏 |
| `created_at` | timestamptz | 创建时间 |

建议唯一约束：`(stat_date, customer_id, product_id)`。
...
### 3.8 客户交易表：`mart.fact_customer_trade`

交易流水表，用于活跃度、偏好、交易金额、交易次数等分析。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `trade_id` | varchar(64) primary key | 交易编号 |
| `trade_date` | date | 交易日期 |
| `trade_time` | timestamptz | 交易时间 |
| `customer_id` | varchar(32) | 客户编号 |
| `product_id` | varchar(32) | 产品编号 |
| `trade_type` | varchar(32) | 买入、卖出、申购、赎回等 |
| `trade_channel` | varchar(32) | APP、柜台、电话等 |
...
### 3.9 客户资产流入流出表：`mart.fact_customer_cash_flow`

客户-日期粒度，用于净流入、流失预警、营销效果分析。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `flow_id` | bigserial primary key | 记录编号 |
| `flow_date` | date | 发生日期 |
| `customer_id` | varchar(32) | 客户编号 |
| `inflow_amount` | numeric(20, 2) | 流入金额 |
| `outflow_amount` | numeric(20, 2) | 流出金额 |
| `net_flow_amount` | numeric(20, 2) | 净流入金额 |
...
- `mart.dim_campaign`：营销活动。
- `mart.fact_marketing_touch`：客户触达、响应、转化记录。
...
| 表 | 建议规模 |
| --- | --- |
| `dim_customer` | 5,000 个客户 |
| `dim_manager` | 120 个服务经理 |
| `dim_product` | 300 个产品 |
| `rel_customer_manager` | 5,500 条关系，包含少量历史关系 |
| `dim_common_code` | 约 80 条枚举 |
| `fact_customer_asset_daily` | 近 180 天，每天约 5,000 条，约 900,000 条 |
| `fact_customer_position_daily` | 近 180 天，每天约 10,000 到 25,000 条 |
| `fact_customer_trade` | 约 80,000 到 150,000 条交易流水 |
| `fact_customer_cash_flow` | 约 200,000 到 500,000 条流水 |
...
| 表 | 索引 |
| --- | --- |
| `dim_customer` | `customer_id`、`customer_level`、`risk_level`、`region_code` |
| `dim_manager` | `manager_id`、`org_id` |
| `rel_customer_manager` | `customer_id`、`manager_id`、`is_current` |
| `dim_product` | `product_id`、`product_type` |
| `fact_customer_asset_daily` | `(stat_date, customer_id)`、`customer_id`、`stat_date` |
| `fact_customer_position_daily` | `(stat_date, customer_id)`、`product_id`、`customer_id` |
| `fact_customer_trade` | `trade_date`、`customer_id`、`product_id`、`trade_type` |
| `fact_customer_cash_flow` | `flow_date`、`customer_id`、`net_flow_amount` |
...
### 6.1 查询主日志：`agent.query_run_log`

每次用户问题对应一条主记录。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `run_id` | uuid primary key | 查询运行 ID |
...
### 6.2 阶段日志：`agent.stage_log`

记录 QueryPlan、SQL、Result 等阶段的输入输出。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `stage_id` | bigserial primary key | 阶段日志 ID |
| `run_id` | uuid | 查询运行 ID |
...
### 6.3 LLM 调用日志：`agent.llm_call_log`

记录模型调用，但不能记录 API Key。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `llm_call_id` | bigserial primary key | LLM 调用 ID |
| `run_id` | uuid | 查询运行 ID |
...
### 6.4 SQL 执行日志：`agent.sql_execution_log`

记录 SQL 是否真正执行成功。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `execution_id` | bigserial primary key | 执行 ID |
| `run_id` | uuid | 查询运行 ID |
...
### 6.5 结果校验日志：`agent.result_validation_log`

记录执行后再次校验的结果。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `validation_id` | bigserial primary key | 校验 ID |
| `run_id` | uuid | 查询运行 ID |
| `execution_id` | bigint | SQL 执行 ID |
...
1. 创建 Alembic 迁移目录。
2. 创建 `mart`、`agent`、`evaluation` schema。
3. 实现 `mart` 业务表 SQLAlchemy 模型。
4. 实现 `agent` 日志表 SQLAlchemy 模型。
5. 编写 deterministic seed 脚本，生成固定规模测试数据。
6. 编写只读 SQL Executor，限制超时、行数和语句类型。
7. 接入 SQL 执行日志和阶段日志。
8. 实现 ResultHardValidator。
9. 接入 ResultCritic。
10. 将结果校验失败反馈回 SQL Actor 修复循环。
11. 后续接入 metadata retriever 和 RAG。
