...
5. 已实现 ResultHardValidator，并将结果硬校验失败反馈回 SQL Actor 修复循环。
6. 已接入 LLM ResultCritic，支持执行后结果语义审核和 SQL 修复反馈。
7. 后续接入分页/异步导出，支持大结果集的受控查看。
8. 后续接入 metadata retriever、RAG、AnswerCritic 和批量评测。

第一阶段完成后，系统就可以走通：

```text
QueryPlan -> SQL -> 审核 -> 执行 PostgreSQL -> 结果硬校验 -> ResultCritic -> 日志记录
```

第二阶段再走通：

```text
元数据召回/RAG -> 更准确的 QueryPlan 和 SQL -> AnswerCritic -> 批量评测
```
